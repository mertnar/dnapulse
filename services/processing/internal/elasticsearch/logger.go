...
// Logger handles Elasticsearch logging for processing service
type Logger struct {
	client      *http.Client
	baseURL     string
	index       string
...
// LogProcessingEvent logs a processing event
func (l *Logger) LogProcessingEvent(ctx context.Context, eventID string, originalEvent, processedEvent map[string]interface{}, rules []string, status string, duration int64, err error) error {
	event := &EventLog{
		EventType: "processing",
		EventID:   eventID,
		Level:     "info",
		Message:   fmt.Sprintf("Event processed: %s", eventID),
	...
	if err != nil {
		event.Level = "error"
		event.Message = fmt.Sprintf("Failed to process event %s: %v", eventID, err)
