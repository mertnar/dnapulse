...
generate_test_events() {
    ...
    local response1=$(curl -s -w "%{http_code}" -X POST "http://localhost:8080/ingest" \
    ...
    local response2=$(curl -s -w "%{http_code}" -X POST "http://localhost:8080/ingest" \
        -H "Content-Type: application/json" \
        -d "$event2")
    ...
    local response3=$(curl -s -w "%{http_code}" -X POST "http://localhost:8080/ingest" \
        -H "Content-Type: application/json" \
        -d "$event3")
...
test_model_inference() {
    ...
    local normal_response=$(curl -s -w "%{http_code}" -X POST "http://localhost:8086/v1/infer" \
        -H "Content-Type: application/json" \
        -d "$normal_request")
    ...
    local anomaly_response=$(curl -s -w "%{http_code}" -X POST "http://localhost:8086/v1/infer" \
        -H "Content-Type: application/json" \
        -d "$anomaly_request")
...
test_configuration() {
  ...
  - id: "error-log-policy"
    ...
    local update_response=$(curl -s -w "%{http_code}" -X PUT "http://localhost:8083/v1/config/decision" \
...
test_observability() {
    ...
    local services=("8080:ingestion" "8081:decision" "8083:config" "8086:model")
